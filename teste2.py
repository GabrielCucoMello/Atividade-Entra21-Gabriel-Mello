...
if __name__ == "__main__":
    ...
    for dayweather in data["next_days"]:
        ...
        y = [x for x in dayweather['max_temp']]
        y2 = [x for x in dayweather['min_temp']]

        print(y)
        new_y = ''.join(y)
        new_y2 = ''.join(y2)
        real_y.append(new_y)
        print(f'{new_y} teste new')
        real_y2.append(new_y2)
        print(f'{real_y} teste real')
        final_y = sorted(real_y)
        print(f'{final_y} teste final')
        final_y2 = sorted(real_y2)


        plt.scatter(x = dayweather['name'], y= final_y, color = 'Red')
        plt.scatter(x = dayweather['name'], y= final_y2, color = 'Blue').


    # real_y.append(new_y)
    # real_y2.append(new_y2)
    # data_final.append(data2)
        
        # plt.scatter.barh(x = dayweather['name'], y = new_y2, color='Blue') 
        # plt.scatter(x = dayweather['name'], y = dayweather['max_temp'], color='Red')
        # plt.scatter.barh(x = dayweather['name'], y = dayweather['min_temp'], color='Blue')


        # plt.scatter(x = dayweather['name'], y= real_y, color = 'Red')
        # plt.scatter(x = dayweather['name'], y= real_y2, color = 'Blue')
    # plt.scatter(x = dayweather['name'], y = new_y, color='Red')
    # plt.scatter(x = dayweather['name'], y = new_y2, color='Blue')

# final_y = sorted(real_y)
# print(f'{final_y} teste')
# final_y2 = sorted(real_y2)
# print(f'{final_y2} teste2')

# print(data_final)

# plt.scatter(x = data_final, y= real_y, color = 'Red')
# plt.scatter(x = data_final, y= real_y2, color = 'Blue')
